...
def serotype_B(pre):   # 大肠+致贺——ectyper
    ...
    subprocess.run(conda_run_command("cm210", f"ectyper -i ./{pre}.final.fasta -c {runtime_nt} -o {pre}_serotype --verify --pathotype"), shell=True)
...
def PathoNet(Pre, species):
    ...
    def _load_vf_hits():
        ...
        return None
    def _mark_focus(stype, focus_list):
        if not stype:
            return '-'
    ...
    if species in {'hinfluenzae', 'vparahaemolyticus', 'listeria', 'ssuis', 'nmeningitidis', 'neisseria'}:
        ...
        if os.path.isfile(f'{Pre}_serotype_result.tsv') and os.path.getsize(f'{Pre}_serotype_result.tsv') != 0:
            ...
            stype = '-'
            for col in ['血清型', '亚型预测']:
                if col in cpserodb.columns and cpserodb[col].tolist():
                    stype = str(cpserodb[col].tolist()[0]).strip()
                    break
            PathoSamdict['血清型'] = _mark_focus(stype, pathodict['serotype'])
...
def serotype_HI(pre):  # 流感嗜血亚型预测——HICAP
    sistr_hicap_env = str(os.environ.get("META_SISTR_HICAP_ENV") or "sistr_hicap").strip() or "sistr_hicap"
    subprocess.run(f'''cut -d '' -f 1 {pre}.final.fasta > {pre}.fasta''', shell=True)
    subprocess.run(conda_run_command(sistr_hicap_env, f"python /home/dell/biosoft/hicap-master/hicap-runner.py -q  {pre}.final.fasta -o ./"), shell=True)
...
def serotype_ys(pre):   # 耶尔森菌
    ...
    def _primary_species_from_checkm() -> str:
        path = Path(f'{pre}.checkm.tsv')
        if not path.is_file() or path.stat().st_size == 0:
            return ''
        try:
            checkm_db = pd.read_table(path).fillna('')
        except Exception:
            return ''
        if checkm_db.empty or '物种名称' not in checkm_db.columns:
            return ''
        raw_value = str(checkm_db.iloc[0].get('物种名称') or '').strip()
        if not raw_value:
            return ''
        return raw_value.split('(')[0].strip()

    def _run_y_enterocolitica_o_typing() -> str | None:
        script_path = project_root / 'scripts' / 'type_yersinia_enterocolitica_o_antigen.py'
        ref_fasta = project_root / 'database' / 'bacteria' / 'yersinia_enterocolitica' / 'o_antigen' / 'reference_o_antigen_markers.fasta'
        ref_manifest = project_root / 'database' / 'bacteria' / 'yersinia_enterocolitica' / 'o_antigen' / 'reference_o_antigen_markers.tsv'
        if not script_path.is_file() or not ref_fasta.is_file() or not ref_manifest.is_file():
            return None
        out_dir = Path(f'{pre}_yersinia_enterocolitica_o_antigen')
        cmd = [
            'python3',
            str(script_path),
            '--input-fasta',
        ...
        summary_path = out_dir / f'{pre}.o_antigen.summary.tsv'
        if not summary_path.is_file() or summary_path.stat().st_size == 0:
            return None
        summary_db = pd.read_table(summary_path).fillna('')
        if summary_db.empty:
            return None
        row = summary_db.iloc[0]
        predicted_o = str(row.get('predicted_o_serotype') or '').strip() or '-'
        status = str(row.get('status') or '').strip() or '-'
        note = str(row.get('note') or '').strip() or '-'
        detected_genes = str(row.get('detected_genes') or '').strip() or '-'
        required_fraction = str(row.get('required_fraction') or '').strip() or '-'
        expected_fraction = str(row.get('expected_fraction') or '').strip() or '-'
        o_result = pd.DataFrame(
            [
                {
                    '样本名称': pre,
                    '物种预测': '小肠结肠炎耶尔森菌',
        ...
        o_result.to_csv(f'{pre}_serotype_result.tsv', sep='\t', index=False)
        return predicted_o

    primary_species = _primary_species_from_checkm().lower()
    if 'yersinia enterocolitica' in primary_species:
        predicted = _run_y_enterocolitica_o_typing()
        if predicted is not None:
            return predicted

    YS_dict = {'样本名': pre, 'PLA': '-', 'YPO2088': '-', 'inv': '-', 'opgG': '-', '97_predict': 'Unknown', '物种预测': '未知耶尔森'}
    subprocess.run(f'cat {pre}.final.fasta|seqkit amplicon -p {_database_path("YS_primer.tsv")} --bed > YS_primer.bed', shell=True)
    if os.path.isfile('YS_primer.bed') and os.path.getsize('YS_primer.bed') != 0:
        ...
        for targene in genelist:
            YS_dict[targene] = '+'
    subprocess.run(f'python {_database_path("Identify_Y.pestis-main", "Identify_Y.pestis", "Identify_Y.pestis_from_dir.py")} {pre}.final.fasta YS_97.txt', shell=True)
...
def serotype_A(pre):  # 沙门氏菌血清型——sistr
    sistr_hicap_env = str(os.environ.get("META_SISTR_HICAP_ENV") or "sistr_hicap").strip() or "sistr_hicap"
    subprocess.run(conda_run_command(sistr_hicap_env, f"sistr -i {pre}.final.fasta {pre} -f tab -o {pre}_result"), shell=True)
    serotype_s = _database_path("pathogenic", "salmonella_52seotype_v2.txt")
...
def serotype_nm(pre):  # 奈瑟氏菌亚型预测——PMGA
    subprocess.run(conda_run_command("amr_aux", f"pmga {pre}.final.fasta  -t 10 --force --blastdir /data1/shanghai_pip/meta_genome/database/pmga/"), shell=True)
...
def serotype_E(pre):  # 霍乱亚型预测
    serodict = {}
    subprocess.run(conda_run_command("choleraefinder", f"python /data/test/test_VP/choleraefinder/choleraefinder.py --input {pre}.final.fasta -o ./ -p /data/test/test_VP/choleraefinder_db/  -t 0.95 -l 0.95 -q"), shell=True)
...
def serotype_st(pre):  # 金葡家系分析——SALTY
    ...
    subprocess.run(conda_run_command("amr_aux", f"salty -i {pre}_st_fafile -o {pre}_st_fafile -t 10"), shell=True)
...
def serotype_lm(pre):  # 单增李斯特血清型——lissero
    subprocess.run(f"{conda_run_command('amr_aux', f'lissero {pre}.final.fasta')} > Lm_sero.tsv ", shell=True)
...
def serotype_SS(Pre):  # 猪链血清型
    database = _database_path("SS_sero")
    ...
    with open('sero.log', 'w') as serof:
        ...
        subprocess.run(f'python {_database_path("SsuisChara", "SsuisChara.py")} -i {Pre}.final.fasta -o {Pre}.SsuisChara.tsv', shell=True)
...
def bp_2037(Pre):
    with open('bp2037.log', 'w') as f:
        ...
        subprocess.run(f'{conda_env_path("ncov", "bin", "vt")} normalize {Pre}.rrn.vcf -r /data1/shanghai_pip/meta_genome/rrn.fasta -o {Pre}.rrn.filt1.vcf', shell=True, stdout=f, stderr=f)
...
def _run_mlst_core(Pre, tSpe):
    ...
    for x in mlst_gene.iloc[0, :].tolist():
        ...
        if '(' in x:
            ...
            try:
                os.system(f"seqkit grep -p {gene}_{gene_num} {conda_env_path('meta_main', 'db', 'pubmlst', mlst_B, f'{gene}.tfa')} >> mlst_all.fa")
            except Exception:
                pass
    ...
    return mlst_B, requested_scheme


def mlst_only(Pre, tSpe):
    print('mlst only')
    mlst_B, _ = _run_mlst_core(Pre, tSpe)
    return mlst_B
