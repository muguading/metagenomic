...
XLSX_PATH = ROOT / "database" / "virus" / "enterovirus" / "typing.xlsx"
OUT_DIR = ROOT / "database" / "virus" / "enterovirus" / "reference_genomes"
GFF_DIR = OUT_DIR / "gff3"
FASTA_MERGED = OUT_DIR / "enterovirus_typing_complete_genomes.fasta"
MANIFEST_PATH = OUT_DIR / "enterovirus_typing_complete_genomes_manifest.tsv"
    ...
    for attempt in range(1, retries + 1):
        try:
            request = urllib.request.Request(
                full_url,
                headers={"User-Agent": "metagenomic-enterovirus-downloader/1.0"},
            )
...
def main() -> int:
    ...
    print(f"Downloaded {len(results)} enterovirus references to {OUT_DIR}")
    return 0
